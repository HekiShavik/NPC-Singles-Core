...
final class LocalCardSearchTool
{
    ...
    public static function enqueue(string $section, string $gameId, string $uiPrefix): void
    {
        ...
        if (is_file($cssPath)) {
            wp_enqueue_style('nps-local-card-search', NPS_CORE_URL . 'assets/local-card-search.css', [], filemtime($cssPath));
        }
        wp_enqueue_script('nps-local-card-search', NPS_CORE_URL . 'assets/local-card-search.js', [], filemtime($jsPath), true);
    ...
    public static function renderSettingsPanel(): void
    {
        ...
        echo '<thead><tr><th>Spil</th><th style="width:220px;">Indekseret</th><th style="width:360px;">Handling</th></tr></thead><tbody>';
    ...
    public static function ajaxSearch(): void
    {
        ...
        $language = strtoupper(trim((string)wp_unslash($_POST['language'] ?? '')));
        $language = (string)apply_filters('nps_local_card_search_language', $language, $gameId);
        $termLength = function_exists('mb_strlen') ? mb_strlen($term) : strlen($term);

        $index = new LocalCardIndex();
        if ($termLength < 2) {
            wp_send_json_success(['items'=>[], 'count'=>0, 'indexed'=>$index->count($gameId, $language)]);
        }

        $items = $index->search($gameId, $term, 60, $language);
        ...
        wp_send_json_success([
            'items' => $items,
            'count' => count($items),
            'indexed' => $index->count($gameId, $language),
            'language' => $language,
        ]);
    }
    ...
    public static function ajaxIndexStart(): void
    {
        ...
        if (!empty($caps['local_card_index'])) {
            wp_send_json_error(['message'=>'Dette spil vedligeholder sit eget søgeindeks. Brug spillets indeksopdatering under Indstillinger.'], 409);
        }
        ...
        if (is_wp_error($sets)) wp_send_json_error(['message'=>$sets->get_error_message()], 502);
        if (!$sets) wp_send_json_error(['message'=>'Ingen sæt kunne findes til indeksering. Opdatér spillets set-liste først.'], 409);
    ...
    public static function ajaxIndexStep(): void
    {
        ...
        if ($cursor >= count($sets)) { self::finishBuild($gameId, $token, $state, $allState); return; }
        ...
        $allState[$gameId] = $state;
        update_option(self::BUILD_OPTION, $allState, false);
        if ((int)$state['cursor'] >= count($sets)) { self::finishBuild($gameId, $token, $state, $allState); return; }
        self::sendProgress($gameId, $state, count($sets), false);
    }
    ...
    private static function loadSets(object $cache, object $api)
    {
        ...
        try {
            $first = (array)$api->get('/sets', ['page'=>1, 'pageSize'=>250, 'orderBy'=>'releaseDate'], 45);
        } catch (\Throwable $e) {
            return new \WP_Error('nps_card_index_sets', 'Kunne ikke hente set-listen: ' . $e->getMessage());
        }
        if (!($first['ok'] ?? false)) return new \WP_Error('nps_card_index_sets', (string)($first['message'] ?? $first['error'] ?? 'Kunne ikke hente set-listen.'));
        ...
        if ($total > count($sets)) {
            ...
            while (count($sets) < $total && $page <= 50) {
                $next = (array)$api->get('/sets', ['page'=>$page, 'pageSize'=>250, 'orderBy'=>'releaseDate'], 45);
                if (!($next['ok'] ?? false)) break;
                $more = self::setsFromApiResponse($next);
                if (!$more) break;
                foreach ($more as $row) $sets[(string)$row['id']] = $row;
    ...
    private static function rowsForSet(array $setInfo, array $cards): array
    {
        ...
        foreach ($cards as $card) {
            ...
            $rows[] = [
                ...
                'search_text'=>implode(' ', array_filter([$name,$version,$oracleName,(string)($setInfo['name'] ?? ''),(string)($setInfo['id'] ?? ''),(string)($setInfo['ptcgoCode'] ?? ''),$number,$artist,$illustrator])),
                'payload'=>[
                    'version'=>$version,
    ...
    {
        foreach ([$card['image'] ?? '', $card['thumbnail'] ?? '', $card['images']['full'] ?? '', $card['images']['large'] ?? '', $card['images']['normal'] ?? '', $card['images']['small'] ?? ''] as $value) {
    ...
    private static function createRuntime($provider)
    {
        ...
        if ($namespace === '') return new \WP_Error('nps_card_index_namespace', 'Spil-pluginet oplyser ikke sit namespace.');
        ...
        if (!class_exists($apiClass) || !class_exists($cacheClass) || !class_exists($setCacheClass)) return new \WP_Error('nps_card_index_runtime', 'Spil-pluginet mangler API/cache-klasser til at bygge søgeindekset.');
        try {
            $api = new $apiClass();
    ...
    private static function assertAjaxAccess(): void
    {
        ...
        if (!current_user_can('manage_woocommerce')) wp_send_json_error(['message'=>'Ingen adgang.'], 403);
    }

    private static function postedGameId(): string
    {
        $gameId = isset($_POST['gameId']) ? sanitize_key(wp_unslash((string)$_POST['gameId'])) : '';
        if ($gameId === '') wp_send_json_error(['message'=>'Spil-id mangler.'], 400);
        return $gameId;
    }
    ...
    private static function providerOrFail(string $gameId)
    {
        ...
        if ($provider === null) wp_send_json_error(['message'=>'Singles-integration blev ikke fundet.'], 404);
        return $provider;
    }
}
